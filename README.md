...
It improves on [looplab/fsm](https://github.com/looplab/fsm), the most popular Go FSM library, by replacing stringly-typed states and events with any comparable type and catching invalid usage at compile time instead of at runtime.
...
`New` panics if you register two transitions for the same `(from, event)` pair. This is always a programmer error and is caught at startup, not at runtime.
...
> **Note:** hooks must not call FSM methods. `Send` holds the write lock for the full transition, so calling it from a hook will deadlock.
